...
type FormData = { organizationName: string; email: string; password: string; confirmPassword: string };
    ...
    <div className="space-y-5">
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
        <div className="space-y-1.5">
          <Label htmlFor="organizationName" style={labelStyle}>Tashkilot nomi</Label>
          <Input
            id="organizationName"
            type="text"
            placeholder="Masalan: ABC O'quv markazi"
            autoComplete="organization"
            style={inputStyle}
            {...register("organizationName")}
          />
          {errors.organizationName && (
            <p style={{ color: "#f87171", fontSize: "12px" }}>{errors.organizationName.message}</p>
          )}
        </div>
