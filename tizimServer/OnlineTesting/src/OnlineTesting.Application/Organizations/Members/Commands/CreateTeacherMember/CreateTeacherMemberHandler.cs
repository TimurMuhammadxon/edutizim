...
public class CreateTeacherMemberHandler : IRequestHandler<CreateTeacherMemberCommand, Guid>
{
    ...
    public CreateTeacherMemberHandler(
    ...
    public async Task<Guid> Handle(CreateTeacherMemberCommand request, CancellationToken ct)
    {
        ...
        var user = User.CreateOrgMember(
            organizationId, Role.Teacher, request.Phone, hash, request.FirstName, request.LastName);
