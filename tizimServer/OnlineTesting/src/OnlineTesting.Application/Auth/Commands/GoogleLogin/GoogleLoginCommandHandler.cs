...
public class GoogleLoginCommandHandler : IRequestHandler<GoogleLoginCommand, AuthResponse>
{
    ...
    private async Task<User> FindOrCreateUserAsync(GoogleAuthData authData, CancellationToken ct)
    {
        ...
        // Новый пользователь — создаём организацию, он становится её OrgAdmin
        var organizationName = string.IsNullOrWhiteSpace(authData.FirstName)
            ? "My Organization"
            : $"{authData.FirstName}'s organization";

        var organization = Organization.Create(organizationName);
        var user = User.CreateOrgAdminFromExternal(authData.Email, organization.Id,
